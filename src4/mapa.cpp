...
Mapa::Mapa(int filaRecibida, int columnaRecibida, char dificultadRecibida){
	...
	this->pMinas->iniciarCursor();
	while(pMinas->avanzarCursor()){
		Mina mina = pMinas->obtenerCursor();
		std::cout<<mina.obtenerFila()<<" "<<mina.obtenerColumna()<<std::endl;
	}
...
void Mapa::mostrarMapa(){
	...
	std::cout<<"casillas desbloqueadas:";
	pCasillasDestapadas->iniciarCursor();

	while(pCasillasDestapadas->avanzarCursor()){

		Casilla actual = pCasillasDestapadas-> obtenerCursor();
		char seDestapo;

		unsigned int fila = actual.obtenerFila();
		unsigned int columna = actual.obtenerColumna();

		char valor = actual.obtenerValor();

		if (actual.seDestapoEnTablero())
				seDestapo = 's';
		else seDestapo = 'n';
		std::cout<<"fila y columna: "<<fila<<" "<<columna<<" valor: "<<valor<<" destapada? "<< seDestapo<<std::endl;
	}
